...
export default function TopUpTab({ network, currency, onSuccess }: TopUpTabProps) {
  ...
  const reserveWalletMutation = useMutation({
    mutationFn: async (net: string) => {
      const res = await apiRequest("POST", "/api/wallets/reserve-for-topup", { network: net });
  ...
  // Pre-select balance from props (network/currency passed from wallet page)
  useEffect(() => {
    if (availableBalances.length > 0) {
      let targetBalance: UserBalance | undefined;

      if (network && currency) {
        targetBalance = availableBalances.find(b => b.network === network && b.currency === currency);
      } else if (network) {
        targetBalance = availableBalances.find(b => b.network === network);
      }
  ...
  }, [cryptoBalances, network, currency]);
  ...
  const handleCopyAddress = async () => {
    ...
    try {
      ...
      setCopied(true);
      toast({ title: t('common.copied'), description: t('topUp.addressCopied') });
      setTimeout(() => setCopied(false), 2000);
    } catch {
      toast({ title: t('common.error'), description: t('topUp.copyError'), variant: "destructive" });
    }
  };

  const handleContinue = () => {
    if (wallet?.address && activeBalance) {
      createTopupMutation.mutate({ walletAddress: wallet.address, network: activeBalance.network });
    }
  };
              ...
              <SelectValue placeholder={t('topUp.selectCrypto')}>
                {activeBalance && (
                  <div className="flex items-center">
                    <img src={getBalanceIcon(activeBalance.id)} alt={activeBalance.title} className="w-8 h-8 rounded-full mr-3" />
              ...
              {availableBalances.map((balance) => (
                ...
                >
                  <div className="flex items-center py-1">
                    <img src={getBalanceIcon(balance.id)} alt={balance.title} className="w-8 h-8 rounded-full mr-3" />
                    <div>
        ...
        <div className="crypto-card text-center">
          {isLoading ? (
            ...
            <>
              ...
              <div className="text-sm text-muted-foreground mb-2">{t('topUp.toWalletAddress')}</div>
              <div className="flex items-center bg-secondary rounded-lg p-3 mb-3">
                <span className="font-mono text-sm flex-1 truncate" data-testid="text-address">{displayValue}</span>
                <button className="ml-2 p-1 hover:bg-white/10 rounded" onClick={handleCopyAddress} data-testid="button-copy-address">
